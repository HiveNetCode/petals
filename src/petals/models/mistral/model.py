...
from transformers.models.mistral import MistralForCausalLM, MistralForSequenceClassification, MistralModel, MistralPreTrainedModel
...
class DistributedMistralModel(FromPretrainedMixin, PTuneMixin, MistralModel):
    """MistralModel, but all transformer layers are hosted by the swarm"""
...
class DistributedMistralForCausalLM(FromPretrainedMixin, RemoteGenerationMixin, MistralForCausalLM):
    _keys_to_ignore_on_load_missing = DistributedMistralModel._keys_to_ignore_on_load_missing
    _keys_to_ignore_on_load_unexpected = DistributedMistralModel._keys_to_ignore_on_load_unexpected

    config_class = DistributedMistralConfig

    def __init__(self, config: DistributedMistralConfig):
        MistralPreTrainedModel.__init__(self, config)
        self.model = DistributedMistralModel(config)
    ...
    @property
    def transformer(self) -> DistributedMistralModel:  # For compatibility with RemoteGenerationMixin
        return self.model


class DistributedMistralForSequenceClassification(FromPretrainedMixin, MistralForSequenceClassification):
    _keys_to_ignore_on_load_missing = DistributedMistralModel._keys_to_ignore_on_load_missing
    _keys_to_ignore_on_load_unexpected = DistributedMistralModel._keys_to_ignore_on_load_unexpected
